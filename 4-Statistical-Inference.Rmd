...
We will rerun means tests with the appropriate contrast arguments, muting the outputs for brevity.
...
Cardinal adds tests to the results object in a wide fashion, with each test having an identifying prefix. Here are the results from the second feature model:
...
We can see that the p-value is large (`r cat("contrast.pairwise.Control.vs.Osteoarthritis.at.Medial_p_value=",h[2]@elementMetadata[[12]], sep = "")`), on the second m/z feature from Hypothesis A above, indicating that we cannot reject the null hypothesis; any difference in mean abundance between conditions in medial tissues is not statistically significant.

### Correct for multiple testing

Running more tests increases the probability that one of the tests will be a false positive. We can correct for this in a variety of ways, such as globally lowering the threshold for significance (e.g., the Bonferroni correction). The preferred method, however, is to correct the false discovery rate (FDR), or the probability that a positive test is actually false. By setting an acceptable rate of positive tests that are false (in our case 0.05, or 5%, per hypothesis), we can apply a less strict correction. Cardinal includes a function for FDR correction, `topFeatures`.
...
We will write a function to perform test hypotheses for each model, but first, let's experiment with different ways to specify contrasts in `emmeans`, using the second feature model.
