...
`Cardinal::contrastTest()` offers arguments to specify custom contrasts using `emmeans` syntax and performed using `emmeans`. First, we will load the `meansTest` results object from 3-Statistical-Modeling. The results object contains a summary data frame accessible with `mtops()` and the `lmer` models. We will also load the results from manual diagnosis of model fit, creating a list of features that adequately meet model assumptions.


```{r load_imzML}
load(file.path("Outputs", "meansTest_cartilage_lmer.RData"))

model_fit_eval <- read_csv(file.path("Outputs", "feature-model-fit-evaluation-utd.csv"), show_col_types = FALSE) |> 
...
In `emmeans`, which Cardinal uses for hypothesis testing and which we will use externally, we specify `~condition * tissue` and supply `"pairwise"` as the `method` to get all pairwise contrasts. `emmeans` offers tremendous functionality and versatility, with [many possible ways to specify contrasts](https://cran.r-project.org/web/packages/emmeans/vignettes/comparisons.html). We will explore other syntax when we repeat these analyses externally from Cardinal in the next part of this workflow.

```{r, results='hide', message=FALSE, warning=FALSE}
measTest_models_with_acceptable_fit <- lmer_means_test[passed$fid]
...
cartilage_models_OK_fit <- cartilage_res[[1]][passed$fid]
```
...
signift_cartilage |> 
  ...
  geom_histogram(binwidth = 0.1, fill = "gray50", color = "white", boundary = 0) +
  facet_wrap(~contrast, labeller = labeller(contrast = c(
    "control_vs_osteoarthritis_in_medial" = "OA vs. Control in Medial",
    "lateral_vs_medial_in_osteoarthritis" = "Medial vs. Lateral in OA"
  ))) +
...
knitr::kable(model_fit_eval |> select(-c(fid)))

m <- readMSIData(file.path("Data","Filtering-Aggregation","Preprocessed-NSFiltered-Aggregated-CartilageFiltered50.imzML"))

stopifnot(length(mz(m))==length(model_fit_eval$fid))
stopifnot(length(mz(m))>=max(passed$fid))

signift_cartilage |> 
  filter(contrast == "control_vs_osteoarthritis_in_medial") |>
  mutate(feature_id = as.integer(feature_id)) |>
  left_join(passed |> 
            mutate(feature_id = as.integer(fid))) |>
  mutate(`M/Z` = round(mz, 3),
          Estimate = round(estimate, 3),
          Std.Error = round(SE, 3),
          `P Value` = round(p.value, 3),
          `FDR Adj. P value` = round(p_value_fdr, 3)) |>
  select(feature_id,`M/Z`, Estimate, Std.Error, `P Value`, `FDR Adj. P value` ) |> 
  arrange(`P Value`) |>
  knitr::kable()
