...
Here is the manual part: as we only have `r length(fids)` models, we can manually diagnose assumption fit for each one using the set of plots in the `./QQ-Itx-SII-Plots` directory. I (the author) have done this manual classification myself, and added my determinations to `Outputs/feature-model-fit-evaluation.csv`. We will refrain from testing our hypotheses on models with poor fit, as we can no longer be sure the tests will yield accurate results.
