...
mse_all_picked <- mse_all_combo |>
            ...
            process(BPPARAM=SnowfastParam(3L)) 
...
bg_isos_mz_approx <- bg_mz_approx[2:7]

bg_idx <- features(m, mz=bg_mz_approx) 

bg_idx_isos <- features(m, mz=bg_isos_mz_approx) 
```

```{r}
intensity(m) <- as.matrix(intensity(m))

m <- subsetFeatures(m, mz %ni% mz(m)[bg_idx_isos])
```
...
In our testing, we found reference had fewer downsides than global constant normalization methods like TIC normalization. TIC normalization assumes few ions are changign between samples. In studies of differential abundance between experimental groups with multiple samples, this assumption is unlikely to be correct. TIC assumes that the same amount of biological sample is being ionized in each pixel. This is also likely to be untrue due to both biology (ionization efficeny due to composition of different tissues) and experimental setup (pixels strattling the end of the sample may have less ionizationable material.) TIC normalization is highly sensitive to outliers. This can be mitgated by using median normalization instead, but median normalaiton can introduce other issues relating to sparsity (see above) when normalizaing features where the median is zero. Finally, on peak picked data with a skewed distributuon of peak intensities, TIC becomes a function of the most intense peaks. This issue, made worse by sparsity and outliers, enforces a strict spatial structure. Reference normalization also has issues. Normaliation is only possible on areas with detectable reference peaks, meaning the concentration and selection of the reference bears heavily on the normalization. References are representitve standards when they are similar to the analyes being measured. This makes references for heterogenous biomolecular classes liek metabolites unrepresentitive and unlikey to produce quality normalization. Finally, without ground truth availible, evalaution of these methods is diffcult. Normalization remains an active area of research. 

We will attach the ROI labels to each pixel for use in later steps. The tissue zones are not subset here - normalization is applied to the full dataset, and tissue areas are subset in downstream steps.
...
Reference normalization divides each spectrum by the reference peak, so pixels where the reference is near the noise floor yield unstable, inflated ratios. Before normalizing, we require the reference to be adequately detected, dropping pixels whose reference intensity falls below a floor. We set the floor at 50, which sits below the 5th percentile of reference intensity in tissue pixels (~110) so genuine signal is retained, while removing the small fraction of pixels (~3% of non-background pixels) at the noise floor and bounding the per-pixel normalization gain to `1000 / 50 = 20`.

```{r}
ref_idx <- features(m, mz = 1570.6763)
ref_intensity <- as.numeric(spectra(m)[ref_idx, ])
m <- subsetPixels(m, ref_intensity >= quantile(ref_intensity, p = c(.025)))
```

```{r}
scale_factor <- round(median(ref_intensity))

normalized <- m |>
        normalize(method = "reference", ref = 1570.6763, scale = scale_factor) |>
        process(BPPARAM=SnowfastParam(workers = 5L)) |>
        summarizePixels(stat = c(tic = "sum", mean = "mean", nnzero = "nnzero"), 
          na.rm = T,  BPPARAM=SnowfastParam(workers = 5L)) |> 
