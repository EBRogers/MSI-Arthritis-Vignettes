...
Preprocessing has many functions but overarching goals include reducing nuisance variation, increasing variation of interest (signal) and enabling comparisons between samples. The choices made during preprocessing can significantly impact the results of downstream analyses. Important steps include loading data, peak recalibration, peak picking, peak alignment, region of interest (ROI) selection, and normalization.
...
# Data availability
This dataset is available on MassIVE, under ID number [MSV000094448](https://doi.org/doi:10.25345/C5HQ3S91K), as both the original dataset from [Schurman et al., 2025](https://doi.org/10.1038/s41413-025-00495-0). This work is presented as a reanalysis. Running this workflow starting with the raw files will produce each intermediate. Alternatively, each dataset and all intermediates can be downloaded and placed in the `./Data` directory.
...
```{r fig-peakpicking-a, results='hide', warning=FALSE, message=FALSE, echo=FALSE}
...
Next let us assign metadata to each pixel. This will assist later with modeling and tissue identification. This notation, i.e. `MSimage$var_name <- value` assigns `value` as a column in the `MSimage` `PositonDataFrame` object. Value must be of length 1 (to assign the same value to every pixel) or length n_pixels (to assign a specific value to each pixel).
...
This chain conducts multiple steps, and yielding a single `MSImagingExperiment` object containing a run for every sample, all with a shared mass axis.

- `combine` groups samples into a single `MSImagingArrays` object.
- `peakPick` performs peak picking on on all spectra with the same method and signal to noise ration (SNR) across all samples
...
The final step, changing the `spectrumRepresentation` ensures that Cardinal write the object to disk as centroided.

After these steps, the resulting combined image will have an orders of magnitude smaller memory footprint, as we are no longer maintaining either separate mass axes for each spectra and non-centroided peaks. The object should easily fit in memory.
...
Cardinal automatically calculates `freq` during peak picking, but this metric is across all runs. In our workflow we recommend filtering peaks by non-zero frequency on a per run basis. The function below can do this. 
...
for (rn in runNames(mse_all_filt)) {
  ...
  writeMSIDataSafe(m,
              file.path("Data", 
                "Preprocessing", 
                "Individial-Picked-Aligned-Filtered", 
                paste0(rn, ".imzML")))
}
gc()
```
...
# ROI Segmentation
 ...
 ROIs define the tissue areas relevant to the biological question to be analyzed in later steps. Region of interest (ROI) segmentation can be achieved though many means. Below we present a method that uses predefined marker ions to segment regions of interest. We will use this method to segment cartilage, bone, and background/extracellular matrix (ECM). It is important involve subject matter experts (i.e., pathologists, biologists who know the tissue) in selection of ROI markers and review of segmentation results. Unsupervised multivariate segmentation methods that use all features, although useful for discovery, should not be used to create ROIs when investigating differential abundance as doing go constitutes double dipping and overfits ROIs to biological variation.
...
First, lets subset a representative image. We will locate the features with m/z values nearest our approximate values for the representative features.
...
-   M/z `r mz(m)[background_feature_ids[7]]` is a highly consistent spatial analog, similar enough to include in ROI segmentation.
...
Sparsity of pixels and features is ubiquitous in MSI. In MSI, Sparsity in MSI is generally a mix of concepts - zero intensity, intensity past the limit of detection, and missing values. Although each is subtly different, they are all highly interlinked concepts due to the nature of mass spectrometry. In practice, they are all the same concept. Unfortunately, these conceptual similarities do not extend to preprocessing. Zero and missing values behave very differently in a mathematical sense. In general, zeros are retained while missing values are dropped. Zeros bias summaries downwards, especially means. Missing values are often dropped, meaning that summaries are biased upwards towards non-missing values. There is no good way around this predicament, other than to reduce sparsity but defining more stringent ROIs and filtering highly sparse features. Regardless, at the time of normalization one will need to choose a method (leaving zeros as they are, or replacing them with NAs) and stick with it through analysis.


## Preparing spectra for median normalization

In our testing, we found median normalization was less biased by outliers than TIC normalization. The trade off is that TIC normalization is *slightly* more robust to sparsity. Median normalization is simple - compute the median intensity for each spectra (pixel) then scale each pixel by that median. Although simple, this still presents issues. If the median of a spectrum is zero, then the scaled intensity becomes undefined. If we replace zeros with missing values, then recompute the median dropping missing values, the median will be biased larger. There is no easy answer. For this analysis, we chose to replace zeros with NA values, as leaving zeros in would result in zero-inflation of mean abundance at the time of statistical modeling. Zero inflation breaks the assumption that residual values are normally distributed. 

Even still, we found that median normalization will still highly biased by outliers. True outliers are rare, but can be of very high magnitude. We chose to clip medians at the 95th percentile to control outliers. These steps are performed in the chunk below using matter's `addProcessing` function to queue steps for parallel processing.
...
We also compute the global median (the median of medians, accross all specta in all runs) to use as a final scaling constant. The final constant can be anything, so we chose this value to remain representitve. 
...
We now apply normalization. Cardinal does not have a native implemenbtation of median normalization, thus we created our own. As the data are centroided and peak picked, they easly fit in memory. 
...
Finally, save the nromalized dataset.
...
```{r fig-peakpicking-b, results='hide', warning=FALSE, message=FALSE, echo=FALSE}
png(filename = file.path("Recreated-Figures", 
