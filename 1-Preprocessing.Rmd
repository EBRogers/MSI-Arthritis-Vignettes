...
#bring into memory to make it faster

intensity(mse_norm) <- as.matrix(intensity(mse_norm))
...
results <- modeling_frame %>%
  group_by(feature_id) %>%
  summarise(
    # t-test for cartilage.mean ~ condition
    t1 = list(tidy(t.test(cartilage.mean ~ condition, var.equal = T))),
    # t-test for cartialge.mean.mvs ~ condition
...
results |>
  ...
  labs(x = "Univariate segmentation of representitive markers: -log10(P value)",
        ...
        fill = "Count") + 
  #ggtitle("P values from T tests between conditions in cartilage\nROIs defined by single ion or multivariate segmentation") +
