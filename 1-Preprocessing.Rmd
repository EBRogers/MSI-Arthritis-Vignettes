...
```{r}
calibrant_mz <- 1570.6896

for (n in names(mse_list_raw)){
  cat(format(Sys.time(), "%Y-%m-%d %H:%M:%S"), ": Starting ",n," SMOOTHING\n")
  m <- mse_list_raw[[n]] |>
...
bg_isos_mz_approx <- bg_mz_approx[2:6]
...
image(m, i = bg_idx, grid = F, key = F, style= "dark", layout = c(6,16))
dev.off()
```
