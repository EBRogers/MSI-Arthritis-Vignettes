...
mse_all_filt <- readMSIData(file.path("Data", 
    "Preprocessing", 
    "Recalibrated-Picked-Aligned-FreqFiltered05perRun.imzML"))

intensity(mse_all_filt) <- as.matrix(intensity(mse_all_filt))

manage_directory(file.path("tmp", "features"))

for (i in 1:length(mz(mse_all_filt))) {
  image_png(mse_all_filt, 
  i = i, 
  key = F, 
  grid = F, 
  style = "dark", 
  ...
  width = 5000, 
  height = 3100, 
  filename = file.path("tmp", 
    "features",
    paste0("feature_", 
        i, 
        "_", 
        round(mz(mse_all_filt)[i],3), 
        ".png")))
}
...
We will start by only normalizing tissue areas.
...
normalized <- m |>
        normalize(ref = 1570.6763, scale = 100000) |>
