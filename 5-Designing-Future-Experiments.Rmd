...
- **In Osteoarthritis: Medial vs Lateral** is a within-subject comparison, so each subject acts as their own control. Therefore, the signal must only overcome the residual (remaining, technical) variation.
...
calc_sample_size <- function(desired_difference,
  ...
  delta <- seq(desired_difference[1], desired_difference[2], 0.001)
...
df_curves <- bind_rows(
  calc_sample_size(c(delta_min, delta_max),
                   alpha_90,
                   power,
...
ggplot(df_curves,
  ...
  scale_x_continuous(labels = scales::percent_format(accuracy = 1), limits = c(0.16, 0.65)) + #
