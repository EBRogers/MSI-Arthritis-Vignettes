...
We will load the combined, normalized, peak-picked image. We will then replace zero's with NAs. ImzML format does not respect NA values, so Cardinal will read them as zeros when reloaded. We will also join the ROIs created in `1-Preprocessing`.
...
Write the intensity matrices for the algorithms. Both algorithms take the file name as arguments. DeepION also requires the a vector of m/z values to compute possible isotope candidates. This will take a second as these files are large.
...
As we are aggregating features by selecting the feature with the highest mean intensity to serve as each isotope group's representative feature.
...
Drop features with greater than 25% sparsity in any run.
...
as_tibble(pData(mse_cartilage)) |> count(run)

as_tibble(fData(mse_cartilage)) |>
  mutate(keep = mz %in% mz(mse_cartilage_filt)) |>
  ggplot(aes(x = mean, y = sd, color = keep)) +
  geom_point() 
```
