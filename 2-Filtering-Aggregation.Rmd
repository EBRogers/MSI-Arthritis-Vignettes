...
Each row of a `MSImagingExperiment` intensity matrix represents a feature, and each column a pixel. Matrix values are intensities. Keep in mind that `MSImagingExperiment` objects usually represent irregularly-shaped samples rastered into a coordinate system, and `Cardinal` only stores the intensities that are actual sample values. This presents an issue: both DeepION and mzClustering require rectangular images.
